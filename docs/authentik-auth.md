...
## 5. Если при нажатии «Sign in» получаете 404

Кнопка ведёт на `https://pulsar.testerhub.ru/api/auth/signin/authentik`. Если в ответ 404:

1. **Reverse proxy (nginx, Caddy и т.п.)** — запросы к `/api/*` должны проксироваться на приложение (порт 3012 или куда смаплен контейнер). Проверьте, что в конфиге нет правил, отдающих 404 для `/api` или не проксирующих этот путь на backend.
2. **Проверка маршрута напрямую** — откройте в браузере `https://pulsar.testerhub.ru/api/auth/signin/authentik` (без параметров). Должен быть редирект на Authentik или 503 «Authentik is not configured». Если 404 — запрос не доходит до Next.js, значит проблема в прокси или в том, что образ собран без этого роута (пересоберите образ и задеплойте заново).
